...
function rand(min,max,dec=0){
  ...
  return dec ? n.toFixed(dec) : Math.floor(n);
}
function telemetry(){
  const latency=rand(28,86);
  const uptime=(99.91+Math.random()*0.08).toFixed(2);
  const packets=rand(980,2999);
  const cpu=rand(18,73);
  const memory=rand(42,79);
  const requests=rand(90,299);
  $("latency").innerHTML=`${latency}<span>ms</span>`;
  $("uptime").innerHTML=`${uptime}<span>%</span>`;
  $("requests").textContent=requests;
  $("telemetry").textContent=`PACKETS ${packets} • CPU ${cpu}% • MEMORY ${memory}% • REGION IN-01 • NODE HEALTH ${uptime}%`;
}
setInterval(telemetry, 3200);
telemetry();
